...
fun ReceiveMoneroScreen(
    ...
    val addressViewModel = viewModel<ReceiveMoneroAddressViewModel>(
        factory = ReceiveMoneroAddressViewModel.Factory(wallet)
    )
    val (adState, _) = rememberAdNativeView(
        BuildConfig.HOME_MARKET_NATIVE,
        adPlacements = "ReceiveMoneroScreen", addressViewModel
    )
